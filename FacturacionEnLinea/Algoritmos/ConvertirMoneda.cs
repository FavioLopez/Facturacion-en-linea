...
namespace Algoritmos
{
    public class ConvertirMoneda
    {
        ...
        public string Convertir(string numero, bool mayusculas)
        {
            ...
            numero = numero.Replace(".", ",");//reemplazar si llega con punto por una coma para hacer funcionar en caso de que no se resueva con .

            //si el numero es entero sin decimales se le asigna 00 
            if (numero.IndexOf(",") == -1)
            {
                numero = numero + ",00";
            }
            //validar el formato ej 0,00 y 999 999 999,00
            r = new Regex(@"\d{1,9},\d{1,2}");
            //obtner el nuemo valor 
            MatchCollection mc = r.Matches(numero);
            if (mc.Count > 0)
            {
                //separar el numero en enteros y decimales
                string[] Num = numero.Split(',');


                //darle formato a la parte decimal
                parte_decimal = Num[1] + "/100" + " " + moneda;
                //convertir en literal los numeros
